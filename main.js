...
(() => {
  ...
  var GROUT_COLOR_TABLES = buildGroutColorTables(GROUT_COLORS);
  var TILE_COLORS_BY_IID = TILE_COLOR_TABLES.byIid;
  var TILE_COLOR_IIDS_BY_ID = TILE_COLOR_TABLES.iidsById;
  var GROUT_COLORS_BY_GID = GROUT_COLOR_TABLES.byGid;
  var GROUT_COLOR_GIDS_BY_ID = GROUT_COLOR_TABLES.gidsById;
  var DEFAULT_STATE = {
    mode: "straight",
    showGrid: false,
  ...
  function tileColor(id, iid, name, value, texture, shadeVariation, swatchVisibility) {
  ...
  function buildGroutColorTables(groutColors) {
    const byGid = /* @__PURE__ */ new Map();
    const gidsById = /* @__PURE__ */ new Map();
  ...
  function serializeRaw(state2) {
    ...
    writer.writeByte(groutColorGid(state2.groutColorId));
  ...
  function deserializeRaw(bytes) {
    ...
    state2.groutColorId = groutColorIdFromGid(reader.readByte());
  ...
  function groutColorGid(colorId) {
  ...
  function groutColorIdFromGid(gid) {
  ...
  var extraPaletteColorId;
  var compactPaletteForViewportHeight = false;
  ...
  var COMPACT_PALETTE_HEIGHT_PX = 960;
  void start();
  ...
  function updatePaletteDensityForViewportHeight({ render: shouldRender }) {
  ...
  function schedulePaletteDensitySync() {
  ...
  function createMaterialSwatch(manufacturerId, colorId, extraClasses = []) {
    const color = paletteColor(colorId);
  ...
  function colorShownInDefaultPalette(colorId) {
    return visibilityShownInDefaultPalette(paletteColor(colorId).swatchVisibility);
  }
  function visibilityShownInDefaultPalette(visibility) {
...
by ${manufacturer.name}`;
  ...
  function renderGroutPalette() {
    ...
    for (const groutColor2 of GROUT_COLORS) {
      if (!visibilityShownInDefaultPalette(groutColor2.swatchVisibility)) {
        continue;
      }
      const swatch = document.createElement("button");
